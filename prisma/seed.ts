...
// Função auxiliar para criar a sessão
async function criarSessao(filmeId: number, data: Date, local: string, tipo: string) {
  ...
  const sessaoCriada = await prisma.sessao.create({
    data: {
      ...
      data,
      horarios,
      assentosOcupados: '[]', // Inicia com array de assentos vazio
    },
  });
...
async function main() {
  ...
  const filmeZootopia = await prisma.filme.create({ data: { titulo: 'Zootopia 2', imagem: '/img/zootopia-2.jpg' } });
  console.log(`Filme criado: ${filmeZootopia.titulo}`);
  const filmeFNAF = await prisma.filme.create({ data: { titulo: 'Five Nights at Freddy\'s 2', imagem: '/img/fnaf2.jpg' } });
  console.log(`Filme criado: ${filmeFNAF.titulo}`);
  const filmeWicked = await prisma.filme.create({ data: { titulo: 'Wicked: Parte 2', imagem: '/img/wicked2.jpg' } });
  ...
  for (let i = 0; i < 5; i++) { // Para hoje e os próximos 4 dias
    ...
    for (const filme of listaFilmes) {
      ...
      await criarSessao(filme.id, new Date(dataBase.setHours(16, 30, 0, 0)), locais[0], tipos[0]);
      await criarSessao(filme.id, new Date(dataBase.setHours(20, 0, 0, 0)), locais[0], tipos[1]);
      await criarSessao(filme.id, new Date(dataBase.setHours(21, 0, 0, 0)), locais[1], tipos[2]);
    }
  }
  ...
  if (primeiraSessao) {
    ...
    console.log(`Reserva de exemplo criada para a sessão ID ${primeiraSessao.id} com assentos ${assentosReservados.join(', ')}`);
  }
